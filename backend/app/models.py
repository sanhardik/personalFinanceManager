...
class StockTrade(Base):
    """
    A single stock trade ingested from a brokerage CSV (e.g. Superhero).

    trade_type values:
      - "Buy"               — purchase of securities
      - "Sell"              — sale of securities
      - "Dividend Received" — dividend payment (quantity/avg_price may be null)

    net_amount: negative for outflows (Buy), positive for inflows (Dividend/Sell).
    trade_hash: SHA256 dedup key = (account_id|trade_date|security_code|trade_type|net_amount).
    """
    ...
    @staticmethod
    def compute_hash(account_id: int, trade_date: str, security_code: str, trade_type: str, net_amount: float) -> str:
        """SHA256 hash for dedup."""
        raw = f"{account_id}|{trade_date}|{security_code}|{trade_type}|{net_amount}"
        return hashlib.sha256(raw.encode()).hexdigest()

    def __repr__(self) -> str:
        return f"<StockTrade {self.trade_date} {self.security_code} {self.trade_type} {self.net_amount}>"


class StockValuation(Base):
    """
    User-entered current price snapshot for a security in a brokerage account.

    Used to compute current_value = price × quantity_held for the holdings table.
    The most recent valuation per (account_id, security_code) is the live price.
    """
